...
public class SpottingUtil {
    public static void trySpot(Player player, @Nullable Entity spottee, IVoiceType voiceType) {
        ...
        spottee.getCapability(SpottingCapability.SPOTTABLE).ifPresent(s -> {
            ...
            traced.set(new TextComponent(spottee.getName().getString()).setStyle(Style.EMPTY).withStyle(y -> y.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_ENTITY,
    ...
    public static Component getSpottingMsg(Component player, Component traced, @Nullable BlockPos hit, String spottedKey, String thereKey) {
        return new TranslatableComponent("chat.type.text", player, new TranslatableComponent(spottedKey,
                new TranslatableComponent(thereKey)
                        .setStyle(Style.EMPTY.withColor(ChatFormatting.YELLOW))
                        .withStyle(s -> {
                            if (hit == null) return s;
                            return s.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponent("[" + hit.getX() + ", " + hit.getY() + ", " + hit.getZ() + "]")));
                        }), traced));
    }
